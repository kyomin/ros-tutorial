...
int main(int argc, char** argv) {   // 퍼블리셔 노드 메인 함수
    ros::init(argc, argv, "topic_publisher");   // 노드명 초기화
    ros::NodeHandle nh;     // ROS 시스템과 통신을 위한 노드 핸들 선언

    /*
        퍼블리셔 선언.
        ros_tutorials_topic 패키지의 MsgTutorial 메시지 파일을 이용한 퍼블리셔 ros_tutorial_pub을 작성한다.
        topic명은 "ros_tutorial_msg"이며, 
        퍼블리셔 큐(queue) 사이즈를 100개로 설정한다는 뜻이다.
    */
